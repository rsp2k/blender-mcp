...
Any process that speaks MCP can be a client. The reference client (`examples/llm_client_example.py`) uses `fastmcp.Client`, which discovers the server's [OAuth endpoints](/reference/auth/) via `/.well-known/oauth-protected-resource`, runs a DCR + PKCE flow, and sets the resulting access token as `Authorization: Bearer …` on every subsequent request.
...
- `/.well-known/oauth-protected-resource` and `/.well-known/oauth-authorization-server` — discovery
- `/{register,authorize,token,revoke}` — the MCP-spec OAuth endpoints (proxied to Authentik in prod, served by an in-memory provider in dev)
- `/` — the Streamable HTTP MCP endpoint produced by `build_http_mcp()`
