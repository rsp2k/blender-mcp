...
1. **Start the server.** Pick one:
   ...
   <TabItem label="Public dev server">

   Skip setup entirely — use the reference deploy:

   ```bash
   export MCP=https://mcp.l.warehack.ing
   ```

   Demo credentials are the same as below. Jump to step 2.
