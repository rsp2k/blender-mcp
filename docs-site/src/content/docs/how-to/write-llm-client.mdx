...
`fastmcp.Client` discovers `/.well-known/oauth-protected-resource/mcp/`, registers via DCR, runs the PKCE flow in your browser, and caches the token to `~/.config/fastmcp/`. Subsequent runs reuse the cached token until it expires.
