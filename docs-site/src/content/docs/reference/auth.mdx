...
<Aside type="note">
The OAuth metadata endpoints are mounted at root (`/.well-known/oauth-protected-resource` and `/.well-known/oauth-authorization-server`), and the resource itself is the server origin (`https://mcp.blender.bet/`). Clients should follow the URLs in the metadata documents verbatim rather than hardcoding paths — the server may add or rename mount points without changing the discovery contract.
</Aside>
